...
export async function GET() {
  ...
  try {
    ...
    const [dbReceiptsHead, marketHealth, tradesResult, eventsResult, receiptsResult, quoteProbe, expirationProbe] =
      await Promise.all([
    ...
    const recentEvents = (eventsResult as any)?.data ?? []
    const recentTrades = (tradesResult as any)?.data ?? []
    ...
    const dbOk = !(dbReceiptsHead as any).error
    const yahooOk = (marketHealth as any)?.status !== "down"
    ...
    return Response.json({
      ...
      checks: {
        db: {
          ok: dbOk,
          receiptsTotal: (dbReceiptsHead as any).count ?? 0,
          error: (dbReceiptsHead as any).error ? String((dbReceiptsHead as any).error) : null,
        },
        yahoo: {
          ok: yahooOk,
          status: (marketHealth as any)?.status,
          latencyMs: (marketHealth as any)?.latencyMs,
          quote: quoteProbe,
      ...
      metrics: {
        tradesLast24h: (tradesResult as any).count ?? 0,
        receiptsLast24h: (receiptsResult as any).count ?? 0,
        eventsLast24h: (eventsResult as any).count ?? 0,
        errorsLast24h: errEvents.length,
        blockedLast24h: blockedEvents.length,
        successRate: (tradesResult as any).count ? Math.round((successfulTrades.length / (tradesResult as any).count) * 100) : 0,
        avgTrustScore: recentTrades.length
          ? Math.round(recentTrades.reduce((sum: number, t: any) => sum + (t.trust_score || 0), 0) / recentTrades.length)
          : 0,
      },
      adapterDiagnostics: {
