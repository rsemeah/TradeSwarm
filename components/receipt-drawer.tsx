    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-50 max-h-[90vh] overflow-hidden rounded-t-2xl bg-card">
        ...
        <section className="mb-4 rounded border border-slate-800 p-3">
          ...
          <p className={`text-lg font-semibold ${verdictColor(pb.finalDecision.action)}`}>{pb.finalDecision.action}</p>
          <p className="text-sm text-slate-200">{pb.finalDecision.reason}</p>
          <p className="mt-2 text-xs text-slate-400">Trust {pb.finalDecision.trustScore} · Recommended ${pb.finalDecision.recommendedAmount ?? 0}</p>
        </section>

        <section className="mb-4 rounded border border-slate-800 p-3">
          <p className="mb-2 text-xs text-slate-400">Preflight gates</p>
          <ul className="space-y-1 text-sm">
            {pb.preflight.gates.map((gate) => (
              <li key={gate.name}>
